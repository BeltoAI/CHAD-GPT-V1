...
async function fetchCompletion(prompt, n_predict, stream) {
    const response = await fetch("http://97.90.193.213:8080/v1/chat/completions", {
        method: 'POST',
        body: JSON.stringify({
            model: "gpt-3.5-turbo",
            messages: [
                {
                    role: "system",
                    content: "You are Belto, an AI assistant. Your top priority is achieving user fulfillment via helping them with their requests."
                },
                {
                    role: "user",
                    content: prompt
